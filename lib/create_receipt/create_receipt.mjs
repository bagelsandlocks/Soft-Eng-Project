...
export const handler = async (event) => {
    ...
    try {
        ...
        const validUser = await new Promise((resolve, reject) => {
            pool.query(
                "SELECT * FROM Shoppers WHERE shopperID = ? AND credential = ?",
                [shopperID, token],
        ...
        const storeRows = await new Promise((resolve, reject) => {
            pool.query(
                "SELECT * FROM Stores WHERE name = ? AND chainName = ?",
                [store, storeChain],
                (err, rows) => err ? reject(err) : resolve(rows)
            );
        });
        ...
        const insertResult = await new Promise((resolve, reject) => {
            pool.query(
                `
                INSERT INTO Receipts (shopperID, date, storeChain, store, receiptName, receipt_id)
                VALUES (?, ?, ?, ?, ?, ?)
                `,
                [shopperID, date, storeChain, store, receiptName, receiptID],
                (err, rows) => err ? reject(err) : resolve(rows)
            );
        });
