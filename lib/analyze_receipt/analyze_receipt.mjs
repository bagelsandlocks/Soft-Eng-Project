...
export const handler = async (event) => {
    ...
    try {
        ...
        if(!event.storeChain)
            throw new Error("Receipt store chain is required.")

        if(!event.store)
            throw new Error("Receipt store is required.")

        const shopperID = event.shopperID;
        const token = event.token;
        const receiptImage = event.file; 
        const date = event.date;
        const store_chain = event.storeChain;
        ...
        const createReceipt = await new Promise((resolve, reject) => {
            pool.query(
                `
                INSERT INTO Receipts (id, name, shopper_id, date, store_chain_name, store_name)
                VALUES (?, ?, ?, ?, ?, ?)
                `,
                [receiptID, receiptName, shopperID, date, store_chain, store],
                (err, rows) => err ? reject(err) : resolve(rows)
            );
        });
        ...
        for (const item of items) {
            ...
            await new Promise((resolve, reject) => {
                pool.query(
                    `
                    INSERT INTO ReceiptItems (id, name, price, quantity, category, receipt_id)
                    VALUES (?, ?, ?, ?, ?, ?)
                    `,
                    [
                        id,
